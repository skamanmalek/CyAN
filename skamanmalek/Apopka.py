...
# Additional input for percentage change
percentage_change_input = st.text_input("Enter Percentage Change (%)", value="0.0")

try:
    # Parse percentage change input
    percentage_change = float(percentage_change_input)
except ValueError:
    st.warning("Please enter a valid numerical value for Percentage Change.")
    st.stop()
...
final_bloom_magnitude_with_percentage_change = final_bloom_magnitude * (1 + percentage_change / 100)
...
st.write(f"Predicted Cyanobacteria Bloom Magnitude with Percentage Change: {final_bloom_magnitude_with_percentage_change:.4f}")
...
chart_data = pd.DataFrame({
    'Magnitude Type': ['Initial Bloom Magnitude', 'Predicted Bloom Magnitude', 'Predicted Bloom Magnitude with Percentage Change'],
    'Magnitude Value': [initial_values['Norm_CyAN'], final_bloom_magnitude, final_bloom_magnitude_with_percentage_change]
})
